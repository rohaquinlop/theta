...
use michin_ai::model::{Model, ModelCatalog, ModelCompat};
...
struct MockProvider {
    ...
    events: std::sync::Mutex<Vec<Result<Vec<AssistantMessageEvent>, MichiNError>>>,
    /// Track call count.
...
impl MockProvider {
    ...
    fn new_with_results(responses: Vec<Result<Vec<AssistantMessageEvent>, MichiNError>>) -> Self {
        Self {
...
mod state {
    use michin_agent_core::{AgentState, RunReport};
    use michin_ai::model::{Model, ModelCompat};
    use michin_ai::{Api, Modality, Provider};
    use std::collections::HashMap;
