...
async fn run_single_turn(
    ...
    loop {
        ...
        if sanitized_through < state.messages.len() {
            ...
            if needs_full {
                let (full, _replay) =
                    michin_ai::sanitize_messages_for_replay(&state.messages, &state.model);
        ...
        .await
        {
            Ok((assistant_msg, stop_reason, unresolved_count, unresolved_calls)) => {
                ...
                if !is_error_response {
                    // Extract cache metrics from API usage before storing.
...
async fn run_llm_stream(
    ...
    (
        ...
        Option<StopReason>,
        usize,
        Vec<michin_ai::UnresolvedToolCall>,
    ),
    AgentError,
> {
    ...
    let mut last_error: Option<michin_ai::MichiNError> = None;
    ...
    Err(AgentError::Llm(
        last_error.unwrap_or(michin_ai::MichiNError::StreamEndedEarly),
    ))
}
