...
pub fn compact_messages(
    ...
    if let Some(summary_msg) = summary {
        ...
        output.push(Message::User {
            content: vec![michin_ai::ContentBlock::text(
...
fn compacted_summary(trimmed: &[Message], trimmed_count: u32) -> Option<Message> {
    ...
    Some(Message::Assistant {
        content: vec![michin_ai::ContentBlock::text(truncate_chars(&text, 1200))],
        api: None,
        provider: None,
        model: None,
        usage: None,
...
fn content_text(content: &[michin_ai::ContentBlock]) -> Option<String> {
    let text = content
        .iter()
