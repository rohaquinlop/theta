...
impl RetryConfig {
    pub fn is_retryable(&self, error: &michin_ai::MichiNError) -> bool {
        matches!(error.class(), michin_ai::ErrorClass::Transient)
    }
}
...
impl ToolCall {
    pub fn from_message(msg: &michin_ai::Message) -> Vec<Self> {
        match msg {
            michin_ai::Message::Assistant { content, .. } => content
                .iter()
