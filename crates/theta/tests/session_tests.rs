...
async fn test_append_missing_entries_preserves_order() {
    ...
    assert_eq!(reopened.messages.len(), 3);
    assert_eq!(
        theta::session::message_fingerprint(&reopened.messages[0]),
        theta::session::message_fingerprint(&a)
    );
    assert_eq!(
        theta::session::message_fingerprint(&reopened.messages[1]),
        theta::session::message_fingerprint(&b)
    );
    assert_eq!(
        theta::session::message_fingerprint(&reopened.messages[2]),
        theta::session::message_fingerprint(&c)
    );
}
