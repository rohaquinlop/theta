...
use michin_ai::model::{Model, ThinkingFormat};
...
impl Provider for OpenAiCompatProvider {
    async fn stream<'a>(
        ...
        let base_url = if model.provider == michin_ai::types::Provider::XiaomiMiMo {
...
pub fn api_key_env(provider: michin_ai::types::Provider) -> &'static str {
    match provider {
        michin_ai::types::Provider::OpenAI => "OPENAI_API_KEY",
        michin_ai::types::Provider::OpenAiCodex => "OPENAI_CODEX_TOKEN",
        michin_ai::types::Provider::DeepSeek => "DEEPSEEK_API_KEY",
        michin_ai::types::Provider::OpenCode => "OPENCODE_API_KEY",
        michin_ai::types::Provider::OpenCodeGo => "OPENCODE_API_KEY",
        michin_ai::types::Provider::XiaomiMiMo => "MIMO_API_KEY",
    }
}
...
pub fn apply_thinking_params(
    body: &mut Value,
    model: &Model,
    level: michin_ai::types::ThinkingLevel,
) {
    ...
    match model.compat.thinking_format {
        Some(michin_ai::model::ThinkingFormat::DeepSeek) => {
            ...
            } else if level == michin_ai::types::ThinkingLevel::Off {
        ...
        Some(michin_ai::model::ThinkingFormat::XiaomiMiMo) => {
            // MiMo: binary on/off only, no reasoning_effort field.
            if level == michin_ai::types::ThinkingLevel::Off {
                body["thinking"] = json!({"type": "disabled"});
