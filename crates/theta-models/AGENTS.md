# theta-models — Agent Rules

> Rules for working on the theta-models crate: built-in model catalog.
...
1. Add definition in `theta-models/src/<provider>.rs`
2. Register in `BuiltInCatalog::new()` in `src/lib.rs`
3. If new provider, update `Provider` enum in `theta-ai/src/types.rs`
