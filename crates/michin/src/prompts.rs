...
//!
//! Templates are Markdown files in `.michin/prompts/` that contain
...
/// Discover all prompt templates from `.michin/prompts/` (project-local only for now).
pub fn discover_templates(working_dir: &Path) -> Vec<PromptTemplate> {
    let prompts_dir = working_dir.join(".michin").join("prompts");
