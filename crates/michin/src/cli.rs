...
/// MichiN: minimal terminal coding agent harness in Rust.
///
/// Running `michin` without a subcommand starts interactive TUI multi-turn mode.
#[derive(Debug, Parser)]
#[command(
    name = "michin",
    version,
    about = "Minimal terminal coding agent harness",
    after_help = "Running `michin` without a subcommand starts interactive TUI mode."
)]
pub struct Cli {
