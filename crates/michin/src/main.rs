//! MichiN entry point: parse CLI, load config, dispatch.

use std::path::{Path, PathBuf};

use clap::Parser;

use michin::cli::{Cli, Command};
use michin::config::MichiNConfig;
use michin::config::load_config;
use michin::interactive::run_tui;
use michin::login::login_provider;
use michin::print_mode::{run_continue_print_mode, run_prompt_print_mode, run_resume_print_mode};
use michin::session::SessionManager;

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    // Initialize logging — write to ~/.michin/michin.log so it doesn't
    ...
    match &cli.command {
        ...
        Some(Command::Rpc) => {
            michin::rpc::run_rpc(&config, &working_dir).await?;
        }
        ...
        None => {
            ...
            let tui_args = michin::cli::TuiArgs { text: vec![] };
...
async fn handle_fork(
    _config: &MichiNConfig,
    working_dir: &Path,
    _cli: &Cli,
    args: &michin::cli::ForkArgs,
) -> anyhow::Result<()> {
    let session_mgr = SessionManager::new(working_dir);
...
async fn handle_tui(
    ...
    working_dir: &Path,
    cli: &Cli,
    args: &michin::cli::TuiArgs,
) -> anyhow::Result<()> {
    // Load persisted settings (last model + thinking from prior sessions).
    let settings = michin::settings::load_settings().await;
