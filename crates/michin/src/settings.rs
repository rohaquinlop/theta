...
//!
//! Stored in `~/.michin/settings.json`. Updated on model switch, thinking
...
impl Default for MichiNSettings {
    fn default() -> Self {
        Self {
            last_session: None,
...
/// Load settings from `~/.michin/settings.json`.
...
pub async fn load_settings() -> MichiNSettings {
    let path = settings_path();
    let contents = match tokio::fs::read_to_string(&path).await {
        Ok(c) => c,
        Err(_) => return MichiNSettings::default(),
    };
...
/// Save settings to `~/.michin/settings.json`.
pub async fn save_settings(settings: &MichiNSettings) -> Result<(), SettingsError> {
    let path = settings_path();
...
mod tests {
    ...
    fn old_flat_model_thinking_map_does_not_crash() {
        ...
        let settings: MichiNSettings = serde_json::from_str(json).expect("old format should parse");
    ...
    fn new_nested_model_thinking_map_parses_correctly() {
        ...
        let settings: MichiNSettings = serde_json::from_str(json).expect("new format should parse");
        assert_eq!(
            settings
                .model_thinking_map
                .get("openai")
    ...
    fn last_thinking_inferred_from_map() {
        let mut settings = MichiNSettings::default();
