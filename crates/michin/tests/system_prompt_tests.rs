...
#[test]
fn response_contract_no_longer_has_michin_extensions_docs() {
    assert!(
...
#[test]
fn extension_guardrails_scopes_to_michin() {
    assert!(EXTENSION_CREATION_GUARDRAILS.contains("their own project"));
    assert!(EXTENSION_CREATION_GUARDRAILS.contains("extend MichiN's behavior"));
}
...
fn ignorable_dirs_reject_vcs_and_build() {
    ...
    assert!(is_ignorable_dir(".michin"));
...
async fn discover_nested_agents_finds_crate_files() {
    ...
    assert!(
        found_crates.contains(&"michin-ai"),
        "missing michin-ai: {:?}",
        found_crates
    );
    assert!(
        found_crates.contains(&"michin-agent-core"),
        "missing michin-agent-core: {:?}",
        found_crates
    );
    assert!(
        found_crates.contains(&"michin-tui"),
        "missing michin-tui: {:?}",
        found_crates
    );
    assert!(
        found_crates.contains(&"michin-models"),
        "missing michin-models: {:?}",
        found_crates
    );
    assert!(
        found_crates.contains(&"michin-script"),
        "missing michin-script: {:?}",
        found_crates
    );
