...
mod edit_tool {
    use michin::tools::edit::make_diff_preview;
...
mod prompts {
    use michin::prompts::PromptTemplate;
...
mod tools {
    use michin::tools::{ToolContext, resolve_path, shorten_path};
    use std::path::{Path, PathBuf};

    #[test]
    fn resolve_path_keeps_absolute_path() {
        let ctx = ToolContext::new(PathBuf::from("/tmp/michin-workdir"));
        let resolved = resolve_path(&ctx, "/Users/rhafid/.michin");
        assert_eq!(resolved, PathBuf::from("/Users/rhafid/.michin"));
    }
    ...
    fn resolve_path_resolves_relative_from_workdir() {
        ...
        assert_eq!(resolved, PathBuf::from("/tmp/michin-workdir/src/main.rs"));
    }

    #[test]
    fn shorten_path_replaces_home_with_tilde() {
        let home = dirs::home_dir().unwrap();
        let path = home.join("projects/michin");
        let result = shorten_path(&path);
        assert!(result.starts_with("~/"));
        assert!(result.ends_with("projects/michin"));
    }

    #[test]
    fn shorten_path_leaves_non_home_path_unchanged() {
        let result = shorten_path(Path::new("/tmp/michin-workdir"));
        assert_eq!(result, "/tmp/michin-workdir");
    }
}

mod rpc {
    use michin::config::{AuthConfig, MichiNConfig, ProviderToken};
    use michin::rpc::resolve_auth_for_model;
    use michin_models::BuiltInCatalog;

    #[tokio::test]
    async fn resolve_auth_for_model_falls_back_to_authenticated_provider() {
        let catalog = BuiltInCatalog::new();
        let mut cfg = MichiNConfig::default();
        cfg.auth = AuthConfig {
        ...
        assert_eq!(model.provider, michin_ai::Provider::OpenAiCodex);
...
mod mentions {
    use michin::mentions::expand_file_mentions;
    use michin::mentions::extract_mentions;
...
mod execution_continuity {
    ...
    use michin_ai::{LlmProvider, MichiNError};
    use tempfile::TempDir;
